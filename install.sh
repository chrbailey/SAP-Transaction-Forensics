#!/usr/bin/env bash
#
# Transaction Forensics - One-Click Installer
#
# This script provides an easy way to get started with Transaction Forensics.
...
setup_project_directory() {
    ...
    if [[ -f "docker-compose.yml" ]] && grep -q "transaction-forensics" docker-compose.yml 2>/dev/null; then
        PROJECT_DIR="$(pwd)"
        ...
        # Default location
        PROJECT_DIR="${HOME}/transaction-forensics"
        ...
        if [[ ! -d "${PROJECT_DIR}" ]]; then
            if [[ "${NON_INTERACTIVE}" == "true" ]]; then
                ...
                echo ""
                echo "Where would you like to install Transaction Forensics?"
                echo ""
...
show_mode_menu() {
    echo ""
    echo -e "${BOLD}${CYAN}=== Transaction Forensics Setup Wizard ===${NC}"
    echo ""
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            --help|-h)
                ...
                ;;
            --version|-v)
                echo "Transaction Forensics Installer v${INSTALLER_VERSION}"
                exit 0
                ;;
            *)
...
show_help() {
    echo "Transaction Forensics - One-Click Installer"
    echo ""
...
main() {
    ...
    log_step "Starting Transaction Forensics installation..."
    echo ""
