...
const VALID_SYSTEMS: ReadonlySet<string> = new Set<string>([
  'SAP', 'NetSuite', 'Salesforce', 'cross-system',
]);
...
export class RuleParser {
   ...
   */
  static parseApprovalText(
    text: string,
  ): { threshold?: number; currency?: string; approverRole?: string } {
   ...
   */
  static parseTimingSLA(
    text: string,
  ): { maxDays?: number; activity?: string } {
  ...
  static createStandardRuleset(systemType: SystemType): WorkflowRule[] {
    ...
    const defs: RuleDefinition[] = systemType === 'SAP'
      ? sapStandardRules()
      : systemType === 'NetSuite'
...
function sapStandardRules(): RuleDefinition[] {
  return [
    ...
    {
      document: 'SOP-GL-001', section: '1.1 - Period Close',
      text: 'No postings allowed to closed fiscal periods',
      system: 'SAP', type: 'field_validation',
      params: { field: 'posting_period', constraint: 'must_be_open' },
    },
    {
      document: 'SOP-GL-001', section: '2.1 - Posting Authorization',
...
function salesforceStandardRules(): RuleDefinition[] {
  return [
    {
      document: 'SF-SALES-001', section: '1.1 - Stage Progression',
      text: 'Opportunities must progress sequentially through stages',
      system: 'Salesforce', type: 'sequence_requirement',
      params: { step1: 'Prospecting', step2: 'Qualification', step3: 'Proposal' },
    },
    {
      document: 'SF-SALES-001', section: '1.2 - Close Date Policy',
      text: 'Close date must not be backdated more than 7 days',
      system: 'Salesforce', type: 'field_validation',
      params: { field: 'close_date', maxBackdateDays: 7 },
    },
    {
      document: 'SF-SALES-001', section: '2.1 - Forecast Category',
    ...
    {
      document: 'SF-SALES-001', section: '2.2 - Pipeline Hygiene',
